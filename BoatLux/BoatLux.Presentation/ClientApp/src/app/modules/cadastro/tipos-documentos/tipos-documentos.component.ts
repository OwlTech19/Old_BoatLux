...
import { RequisicaoBuscaTipoDocumentoModel, TipoDocumentoModel, RequisicaoApagarTipoDocumentoModel, RequisicaoRelatorioFinanceiroTiposDocumentoModel } from '../../../interfaces/cadastro.interface';
...
import { TiposDocumento_CadastroDialogComponent } from './cadastro-dialog/cadastro-dialog.component';
...
export class TiposDocumentosComponent implements OnInit, OnDestroy {
    ...
    itensDataSource = new MatTableDataSource<TipoDocumentoModel>();
    ...
    novoItem() {
        ...
        const dialog = this._matDialog.open(TiposDocumento_CadastroDialogComponent, {
            width: '500px',
            data: { item: null },
            disableClose: true
        });
    ...
    editarItem(item: TipoDocumentoModel) {
    ...
    apagarItem(item: TipoDocumentoModel) {
        ...
        Swal.fire({
            ...
            if (result.value) {
                let requisicao: RequisicaoApagarTipoDocumentoModel =
                {
                    id: item.id
                };
                this._cadastroService.financeiro_ApagarTiposDocumentos(requisicao).subscribe((resultado) => {
    ...
    imprimirBusca(formato: number, visualizar: boolean = false): void {
        ...
        let requisicao: RequisicaoRelatorioFinanceiroTiposDocumentoModel =
        {
            formato: formato,
            busca:
            {
        ...
        this._cadastroService.financeiro_gerarRelatorioTiposDocumento(requisicao, 'Relatório de Tipos de Documento', visualizar);
    }
    ...
    buscarItens(): void {
        ...
        let requisicao: RequisicaoBuscaTipoDocumentoModel =
        {
            filtroId: this.buscaFormGroup.value.filtroId,
            filtroDescricao: this.buscaFormGroup.value.filtroDescricao
        };

        this._cadastroService.financeiro_BuscarTiposDocumentos(requisicao).subscribe((resultado) => {
